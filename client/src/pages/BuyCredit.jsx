...
const BuyCredit = () => {
  ...
  const initPay = async (order) =>{
    const options = {
      ...
      handler: async (response) => {
        try {
          const {data} = await aixos.post(backendUrl + 'api/user/verify-razor', 
  ...
  const paymentRazorpay = async (planId) => {
    try {
      ...
      const {data} = await aixos.post(backendUrl + 'api/user/pay-razor', {planId},
      {headers: {token}})
      ...
      <div className='flex flex-wrap justify-center gap-6 text-left'>
        {plans.map((item, index)=>(
          ...
          px-8 text-gray-600 hover:scale-105 transition-all duration-500'>
            <img width={42} src={assets.logo_icon} alt=""/>
